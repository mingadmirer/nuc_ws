...
bool detection_interfaces__msg__detection_result__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // depth_mm
    PyObject * field = PyObject_GetAttrString(_pymsg, "depth_mm");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->depth_mm = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // timestamp
...
PyObject * detection_interfaces__msg__detection_result__convert_to_py(void * raw_ros_message)
{
  ...
  {  // depth_mm
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->depth_mm);
    {
      int rc = PyObject_SetAttrString(_pymessage, "depth_mm", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
