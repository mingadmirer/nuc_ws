...
// ── 析构 ──
BlueMissionNode::~BlueMissionNode() { serial_.close(); }

// ── 构造 ──
BlueMissionNode::BlueMissionNode(const rclcpp::NodeOptions& opts)
    : Node("blue_mission", opts)
{
    // ── 开环运动：速度 + 时长 ──
    initial_right_speed_     = declare_parameter("initial_right_speed", 2000);
    initial_right_time_      = declare_parameter("initial_right_time_s", 4.0);
    ...
    sub_det_ = create_subscription<detection_interfaces::msg::DetectionArray>(
        "/detections", 10,
        std::bind(&BlueMissionNode::onDetection, this, std::placeholders::_1));

    // ── 主循环 10Hz ──
    tick_timer_ = create_wall_timer(100ms, std::bind(&BlueMissionNode::onTick, this));

    RCLCPP_INFO(get_logger(), "=== 崇武探幽 v1 蓝队 启动 ===");
    sendWristAngle(120);  // 夹爪复位到120
    transitionTo(INIT);
}

// ── 状态名 ──
std::string BlueMissionNode::stateName(State s) {
    switch (s) {
...
// ── 状态转移 ──
void BlueMissionNode::transitionTo(State s) {
    ...
    case ALIGN_RACK: {
        ...
        delay_timer_ = create_wall_timer(3.05s, [this]() {
            delay_timer_->cancel();
...
void BlueMissionNode::onDetection(const detection_interfaces::msg::DetectionArray::SharedPtr msg) {
...
// ── 夹爪 ──
void BlueMissionNode::sendGrip()       { sendFrameReliable(CommandEncoder::encodeGrip()); }
void BlueMissionNode::sendRelease()    { sendFrameReliable(CommandEncoder::encodeRelease()); }
void BlueMissionNode::sendWristAngle(int deg) {
    sendFrameReliable(CommandEncoder::encodeWristAngle(static_cast<uint8_t>(deg)));
}

// ── STM32 握手（阻塞轮询直到就绪）──
bool BlueMissionNode::handshakeSTM32() {
...
bool BlueMissionNode::sendFrameReliable(const std::vector<uint8_t>& frame) {
    // TODO: 暂时注释重试等待应答逻辑
...
int main(int argc, char** argv) {
    ...
    rclcpp::spin(std::make_shared<BlueMissionNode>());
    rclcpp::shutdown();
    return 0;
}
